...
- 🤖 **TTS 服务**: 支持 TTS-API 和 TTS-API-Lite 两种 TTS 服务
...
- ✅ **TTS 服务支持**
  - **TTS-API-Lite**: 轻量级 TTS 在线生成 API 系统，建议普通服务器安装
  - **TTS-API**: 支持在线 TTS 和本地 TTS 生成，对 GPU 硬件要求较高
  - 在线 Edge-TTS 语音朗读（Beta，仅供体验）

### 📄 Office 文档支持

- ✅ **Office 文档浏览**
  - 支持 Word (.docx) 文档上传和浏览
  - 支持 Excel (.xlsx) 文档上传和浏览
  - 简单文档浏览功能
...
1. **TTS-API-Lite（轻量 TTS 在线生成 API 系统）**
   - 轻量级 TTS 服务，建议普通服务器安装
   - 提供在线 TTS 生成 API
   - 资源占用低，适合生产环境
...
3. **Office 文档支持**
   - 增加 Office 文档（Word、Excel、PowerPoint）的上传功能
   - 支持简单文档浏览功能

4. **笔记导出和导入增强**
   - 笔记及高亮内容可导出为 Markdown
   - 支持将导出的笔记作为新书籍添加到服务器

5. **语音朗读功能（Beta）**
   - 利用在线 Edge-TTS 实现语音朗读
   - 目前为 Beta 版本，仅供体验
   - 后续将继续修复完善

#### 🐛 修复内容

1. **修复 Ollama 本地服务无法调用问题**
   - 修复了本地 Ollama 服务连接问题
   - 改进了服务发现和连接机制
