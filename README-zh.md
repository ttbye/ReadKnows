# 📚 ReadKnows (读士私人书库)

<div align="right">
<sub>[English](README.md) | [中文](README-zh.md)</sub>
</div>

<div align="center">

![Version](https://img.shields.io/badge/version-2.3.261.beta-blue.svg)
![License](https://img.shields.io/badge/license-Apache%202.0-blue.svg)
![Docker](https://img.shields.io/badge/docker-ready-blue.svg)
![Platform](https://img.shields.io/badge/platform-macOS%20%7C%20Windows%20%7C%20Linux%20%7C%20NAS-lightgrey.svg)

**支持多格式、多平台、多用户的私人电子书管理平台**

[功能特性](#-功能特性) • [快速开始](#-快速开始) • [安装部署](#-安装部署) • [使用文档](#-使用文档)

</div>

---

## 📖 项目简介

友情提醒：本系统仅供个人学习和图书、文档资料管理使用！

> 💡 **特别提示**：
> - Android APP 在开放式墨水屏电子书（如：掌阅电纸书等）上安装后使用良好，为墨水屏阅读提供了优秀的阅读体验。
> - iPad 或 iPhone 等 iOS 设备可通过 PWA（渐进式 Web 应用）使用。无需通过 App Store 安装，直接在 Safari 浏览器中访问网站并添加到主屏幕即可使用。

ReadKnows（读士私人书库）是一个功能强大的私人电子书管理平台，支持 EPUB、PDF、TXT、MOBI、docx、md 等多种格式的电子书阅读和管理。采用现代化的 Web 技术栈，提供流畅的阅读体验和强大的管理功能。

### ✨ 核心特性

- 📚 **多格式支持**: EPUB、PDF、TXT、MOBI、DOCX、MD
- 🌐 **多平台访问**: Web、iOS (PWA)、Android (APK)、macOS、Windows
- 👥 **多用户系统**: 个人书架和阅读进度独立管理
- 📱 **PWA 支持**: 可安装到设备主屏幕
- 🎨 **专业阅读器**: 内置专业的 EPUB 和 PDF 阅读器，支持多种阅读主题
- 🔍 **智能搜索**: 支持全文搜索，快速定位内容
- 📊 **阅读统计**: 详细的阅读历史和进度统计
- 🤖 **AI 集成**: 支持 Ollama 等 AI 服务，提供智能阅读辅助
- 📦 **批量导入**: 支持批量扫描和导入本地书籍
- 🔐 **安全可靠**: JWT 认证，IP 访问控制，数据加密存储
- 📝 **笔记管理**: 支持阅读笔记、高亮标记，支持导出和导入为笔记书
- 🏷️ **分类管理**: 支持书籍分类和标签管理
- 📡 **OPDS 支持**: 支持 OPDS 协议，兼容主流阅读器
- 🔊 **语音朗读**: 支持在线 TTS 语音朗读功能（Beta）
- 📄 **Office 文档**: 支持 Office 文档（Word、Excel、PowerPoint）的上传和浏览
- 🎧 **有声小说**: 支持有声书/有声小说的导入、管理与播放
- 💬 **书友交流**: 内置书友系统与消息中心，支持好友、书友会群聊、文件与书籍分享

---

## 🚀 功能特性

### 📖 阅读功能
- 多格式阅读器（EPUB、PDF、TXT、MOBI、DOCX、MD）
- 多种阅读主题（浅色、深色、护眼、绿色）
- 字体大小、字体类型、行间距自定义
- 支持自定义字体（可上传字体文件）
- 目录导航（支持多级目录）
- 书签管理
- 笔记和高亮（离线可用，联网自动同步）
- 全文搜索（支持正则表达式）
- 阅读进度同步（多设备同步）

### 📚 管理功能
- 上传、删除、编辑书籍信息
- 自动提取书籍元数据（标题、作者、封面等）
- 支持豆瓣 API 自动获取书籍信息
- 书籍分类和标签管理
- 批量操作（批量删除、批量分类）
- 个人书架管理
- 批量导入（支持文件监控自动导入）
- 支持子目录递归扫描

### 👥 用户功能
- 多用户支持
- 用户注册、登录、权限管理
- 管理员后台
- 阅读统计（阅读时长、阅读进度、阅读历史）
- 个人设置

### 🤖 AI 功能
- 支持 Ollama 本地 AI 服务
- 智能阅读辅助
- 书籍内容分析
- AI 对话功能

### 🔐 安全功能
- JWT 身份认证
- IP 访问控制（白名单/黑名单）
- 验证码保护（登录、注册）
- 密码加密（bcrypt）

---

## 🐳 安装部署

### 前置要求

- **Docker**: 20.10+ 和 Docker Compose 2.0+
- **内存**: 至少 2GB 可用内存（推荐 4GB+）
- **磁盘空间**: 至少 5GB 可用磁盘空间（推荐 10GB+）
- **网络**: 需要访问互联网（用于下载镜像和依赖）

### 系统要求

- **macOS**: 10.15+ (Catalina 或更高版本) 需要 Docker Desktop
- **Windows**: 10/11 (64位) 需要 WSL 2 或 Docker Desktop
- **Linux**: Ubuntu 20.04+ / Debian 11+ / CentOS 8+ / Fedora 34+ 需要 Docker Engine
- **群晖 NAS**: DSM 7.0+ 或 DSM 6.2+ 需要 Docker 套件

---

## 🚀 快速开始（Docker 安装）

### 方式一：一键安装脚本（推荐）

这是最简单快捷的安装方式，脚本会自动完成所有配置。

#### 步骤 1：克隆项目

```bash
git clone https://github.com/ttbye/readknows.git
cd readknows
```

#### 步骤 2：运行安装脚本

```bash
# 赋予执行权限
chmod +x install-zh.sh

# 运行安装脚本
./install-zh.sh
```

安装脚本会自动执行以下操作：
1. 检查 Docker 和 Docker Compose 是否安装
2. 检查 Docker 服务是否运行
3. 自动检测操作系统平台（macOS/Windows/Linux/群晖）
4. 自动创建 `.env` 环境变量文件（生成随机 JWT_SECRET）
5. 创建数据目录结构
6. 检查端口是否被占用（1280 前端，1281 后端）
7. 构建 Docker 镜像（如果需要）
8. 启动服务容器
9. 初始化管理员账户（可选）

#### 步骤 3：访问应用

安装完成后，您会看到：

```
========================================
安装完成
========================================
✅ ReadKnows (读士私人书库) 已成功安装并启动！

访问地址:
  🌐 前端: http://localhost:1280
  🔌 后端API: http://localhost:1281

默认管理员账户:
  用户名: books
  密码: readknows

⚠️  首次登录后请立即修改默认密码！
```

打开浏览器访问：**http://localhost:1280**

---

### 方式二：手动 Docker 部署

如果您想手动控制部署过程，可以按照以下步骤操作。

#### 步骤 1：克隆项目

```bash
git clone https://github.com/ttbye/readknows.git
cd readknows
```

#### 步骤 2：创建环境变量文件

在项目根目录创建 `.env` 文件：

```bash
cat > .env << EOF
# JWT配置
JWT_SECRET=$(openssl rand -base64 32)
JWT_EXPIRES_IN=7d

# 豆瓣API配置（可选）
DOUBAN_API_BASE=

# AI配置（可选）
AI_PROVIDER=ollama
AI_API_URL=http://frontend:1280/ollama-proxy
AI_API_KEY=
AI_MODEL=llama2

# Ollama服务器地址
OLLAMA_URL=http://host.docker.internal:11434

# CORS配置（公网域名访问必须配置）
# 请将 '您的域名' 替换为您的实际域名
# 多个域名可以用逗号分隔
ALLOWED_ORIGINS=https://您的域名:1280
EOF
```

> ⚠️ **重要提示**：如果部署到公网域名，请将 `ALLOWED_ORIGINS` 中的 `您的域名` 替换为您的实际域名。本地开发可以省略此配置。

#### 步骤 3：创建数据目录

```bash
# 创建数据目录结构
mkdir -p data/backend/{data,books,covers,fonts,import}
```

#### 步骤 4：选择 docker-compose 文件

根据您的平台选择对应的配置文件：

- **标准环境**: `sh/docker-compose.yml`
- **macOS**: `sh/docker-compose-MACOS.yml`
- **Windows**: `sh/docker-compose-WINDOWS.yml`
- **Linux**: `sh/docker-compose-Linux.yml`
- **群晖 NAS**: `sh/docker-compose-Synology.yml`

#### 步骤 5：构建并启动服务

```bash
# 进入 sh 目录
cd sh

# 使用标准配置构建并启动
docker compose -f docker-compose.yml up -d --build

# 或使用平台特定配置（以 Linux 为例）
docker compose -f docker-compose-Linux.yml up -d --build
```

#### 步骤 6：查看服务状态

```bash
# 查看容器状态
docker compose -f docker-compose.yml ps

# 查看日志
docker compose -f docker-compose.yml logs -f

# 只查看后端日志
docker compose -f docker-compose.yml logs -f backend

# 只查看前端日志
docker compose -f docker-compose.yml logs -f frontend
```

#### 步骤 7：初始化管理员账户

```bash
# 执行初始化脚本
docker compose -f docker-compose.yml exec backend node scripts/initAdmin.js

# 或使用项目根目录的脚本
cd ..
chmod +x sh/init-admin.sh
./sh/init-admin.sh
```

#### 步骤 8：访问应用

打开浏览器访问：**http://localhost:1280**

使用默认凭证登录：
- **用户名**: `books`
- **密码**: `readknows`

> ⚠️ **安全提示**: 首次登录后请立即修改默认密码！

---

## ⚙️ 配置说明

### 环境变量配置

`.env` 文件包含以下配置项：

```env
# JWT 认证配置
JWT_SECRET=your-secret-key-here-change-in-production
JWT_EXPIRES_IN=7d

# 豆瓣 API 配置（可选）
DOUBAN_API_BASE=https://api.douban.com/v2

# AI 配置（可选）
AI_PROVIDER=ollama
AI_API_URL=http://frontend:1280/ollama-proxy
AI_API_KEY=
AI_MODEL=llama2

# Ollama 服务器配置
OLLAMA_URL=http://host.docker.internal:11434

# CORS 配置（公网域名访问必须配置）
# 格式：多个域名用逗号分隔
# 示例：https://example.com:1280,https://www.example.com:1280
# 如果不配置，默认只允许本地网络访问
ALLOWED_ORIGINS=https://您的域名:1280
```

> ⚠️ **重要提示**：如果您部署到公网域名，**必须**在 `.env` 文件或 docker-compose 配置文件中配置 `ALLOWED_ORIGINS`。请将 `您的域名` 替换为您的实际域名。多个域名可以用逗号分隔。

### 数据目录结构

默认数据目录结构：

```
data/
└── backend/
    ├── data/          # SQLite 数据库文件
    ├── books/         # 书籍文件存储目录
    ├── covers/        # 封面图片存储目录
    ├── fonts/         # 字体文件存储目录
    └── import/        # 导入目录（监控此目录自动导入）
```

### 端口配置

默认端口配置：
- **前端**: 1280 (HTTP)
- **后端 API**: 1281 (HTTP)

修改端口：在 `docker-compose.yml` 中修改端口映射：

```yaml
services:
  frontend:
    ports:
      - "8080:1280"  # 将前端端口改为 8080
  backend:
    ports:
      - "8081:1281"  # 将后端端口改为 8081
```

### CORS 配置（公网域名访问）

如果您将 ReadKnows 部署到公网域名（不仅仅是 localhost），**必须**配置 `ALLOWED_ORIGINS` 环境变量以允许来自您域名的 CORS 请求。

#### 方法一：在 `.env` 文件中配置

在 `.env` 文件中添加：

```env
ALLOWED_ORIGINS=https://您的域名:1280
```

多个域名配置：

```env
ALLOWED_ORIGINS=https://example.com:1280,https://www.example.com:1280,https://app.example.com:1280
```

#### 方法二：在 docker-compose 文件中配置

如果您使用平台特定的 docker-compose 文件（例如 `docker-compose-MACOS.yml`、`docker-compose-Linux.yml`），可以直接在文件中修改 `ALLOWED_ORIGINS` 环境变量：

```yaml
environment:
  - ALLOWED_ORIGINS=${ALLOWED_ORIGINS:-https://您的域名:1280}
```

请将 `您的域名` 替换为您的实际域名。

#### 重要提示：

- **本地开发**：如果仅通过 `localhost` 或本地网络访问，可以省略此配置。
- **公网域名**：如果部署到公网域名，此配置是**必需的**，否则会出现 CORS 错误。
- **端口号**：确保 `ALLOWED_ORIGINS` 中的端口号与您的前端端口匹配（默认为 1280）。
- **HTTPS**：如果使用 HTTPS，URL 中使用 `https://`。如果使用 HTTP，使用 `http://`。
- **多个域名**：多个域名用逗号分隔（不要有空格）。

---

## 📖 使用文档

### 首次登录

安装完成后，使用默认管理员账户登录：
- **用户名**: `books`
- **密码**: `readknows`
- **邮箱**: `admin@readknows.local`

> ⚠️ **安全提示**: 首次登录后请立即修改默认密码！

### 安装 Calibre（MOBI 转 EPUB）

如果需要 MOBI 转 EPUB 功能，需要安装 Calibre：

```bash
chmod +x sh/install-calibre.sh
./sh/install-calibre.sh
```

### 批量导入书籍

#### 方法一：通过 Web 界面导入
1. 登录系统
2. 进入 **上传** 页面
3. 点击 **批量导入** 或 **扫描导入**
4. 选择要导入的目录

#### 方法二：文件监控自动导入
1. 将书籍文件放入 `data/backend/import/` 目录
2. 系统会自动监控此目录
3. 检测到新文件后自动导入

### 常用管理命令

```bash
# 查看服务状态
docker compose -f sh/docker-compose.yml ps

# 查看日志
docker compose -f sh/docker-compose.yml logs -f

# 重启服务
docker compose -f sh/docker-compose.yml restart

# 停止服务
docker compose -f sh/docker-compose.yml stop

# 启动服务
docker compose -f sh/docker-compose.yml start

# 停止并删除容器
docker compose -f sh/docker-compose.yml down

# 重建镜像
docker compose -f sh/docker-compose.yml build --no-cache
```

---

## 🐛 常见问题

### 容器启动失败

1. **检查 Docker 服务**:
   ```bash
   docker info
   ```

2. **检查端口是否被占用**:
   ```bash
   # Linux/macOS
   lsof -i :1280
   lsof -i :1281
   
   # Windows
   netstat -ano | findstr :1280
   netstat -ano | findstr :1281
   ```

3. **查看容器日志**:
   ```bash
   docker compose -f sh/docker-compose.yml logs backend
   docker compose -f sh/docker-compose.yml logs frontend
   ```

### EPUB 封面无法提取/显示

通常是目录权限问题：

```bash
# 修复权限
sudo chmod -R 777 data/backend/covers

# 重启容器
docker compose -f sh/docker-compose.yml restart backend
```

### 无法访问宿主机服务（如 Ollama）

检查网络配置：

1. **使用 host.docker.internal**:
   ```env
   OLLAMA_URL=http://host.docker.internal:11434
   ```

2. **Linux 需要额外配置**:
   ```yaml
   # 在 docker-compose.yml 中添加
   extra_hosts:
     - "host.docker.internal:host-gateway"
   ```

### MOBI 文件无法转换

需要安装 Calibre：

```bash
./sh/install-calibre.sh
```

### 忘记管理员密码

重置密码：

```bash
docker compose -f sh/docker-compose.yml exec backend node scripts/resetPassword.js
```

---

## 🔒 安全说明

### 生产环境安全建议

1. **修改默认密码**: 首次登录后立即修改默认管理员密码
2. **使用强密码**: JWT_SECRET 应使用强随机字符串
3. **配置 HTTPS**: 建议使用 Nginx 反向代理配置 HTTPS
4. **定期备份**: 定期备份数据库和书籍文件
5. **IP 访问控制**: 配置 IP 白名单限制访问
6. **更新依赖**: 定期更新依赖包修复安全漏洞
7. **防火墙配置**: 配置防火墙规则限制端口访问

---

## 📄 许可证

本项目采用 [Apache License 2.0](LICENSE) 许可证。

---

## 🙏 致谢

- [EPUB.js](https://github.com/futurepress/epub.js) - EPUB 阅读器
- [PDF.js](https://github.com/mozilla/pdf.js) - PDF 阅读器
- [Calibre](https://calibre-ebook.com/) - 电子书管理工具
- [React](https://react.dev/) - UI 框架
- [Express](https://expressjs.com/) - Web 框架

---

## 📮 联系方式

- **作者**: ttbye
- **项目地址**: https://github.com/ttbye/readknows
- **问题反馈**: https://github.com/ttbye/readknows/issues

---

<div align="center">

**Made with ❤️ by ttbye**

[⬆ 回到顶部](#-readknows-读士私人书库)

</div>
