...
router.get('/public', async (req, res) => {
    try {
        ...
        const privateAccessKey = db_1.db.prepare('SELECT value FROM system_settings WHERE key = ?').get('private_access_key');
        res.json({
...
router.get('/', auth_1.authenticateToken, async (req, res) => {
    try {
        const settings = db_1.db.prepare('SELECT * FROM system_settings ORDER BY key').all();
        const settingsObj = {};
...
router.put('/:key', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        // 检查设置是否存在
        const existing = db_1.db.prepare('SELECT id FROM system_settings WHERE key = ?').get(key);
        if (existing) {
            // 更新现有设置
            db_1.db.prepare('UPDATE system_settings SET value = ?, updated_at = CURRENT_TIMESTAMP WHERE key = ?').run(value, key);
        }
        else {
            // 创建新设置
            const id = (0, uuid_1.v4)();
            db_1.db.prepare('INSERT INTO system_settings (id, key, value) VALUES (?, ?, ?)').run(id, key, value);
        }
        // 如果是路径设置，验证路径是否存在
        if (key === 'books_storage_path' || key === 'books_scan_path') {
            if (value && !fs_1.default.existsSync(value)) {
                try {
        ...
        const setting = db_1.db.prepare('SELECT * FROM system_settings WHERE key = ?').get(key);
...
// 获取所有书籍类型
router.get('/book-categories', auth_1.authenticateToken, async (req, res) => {
    try {
...
// 创建书籍类型（仅管理员）
router.post('/book-categories', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        // 检查是否已存在
        const existing = db_1.db.prepare('SELECT id FROM book_categories WHERE name = ?').get(categoryName);
        if (existing) {
    ...
    `).run(id, categoryName, order);
        const category = db_1.db.prepare('SELECT * FROM book_categories WHERE id = ?').get(id);
...
// 更新书籍类型（仅管理员）
router.put('/book-categories/:id', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const { name, display_order } = req.body;
        // 检查书籍类型是否存在
        const existing = db_1.db.prepare('SELECT * FROM book_categories WHERE id = ?').get(id);
        if (!existing) {
            return res.status(404).json({ error: '书籍类型不存在' });
        }
        // 如果提供了新名称，检查是否与其他类型冲突
        if (name && name.trim() !== existing.name) {
            const nameConflict = db_1.db.prepare('SELECT id FROM book_categories WHERE name = ? AND id != ?').get(name.trim(), id);
            if (nameConflict) {
...
// 删除书籍类型（仅管理员）
router.delete('/book-categories/:id', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        // 检查书籍类型是否存在
        const existing = db_1.db.prepare('SELECT * FROM book_categories WHERE id = ?').get(id);
        ...
        // 检查是否有书籍使用此类型
        const booksUsingCategory = db_1.db.prepare('SELECT COUNT(*) as count FROM books WHERE category = ?').get(existing.name);
        ...
        db_1.db.prepare('DELETE FROM book_categories WHERE id = ?').run(id);
...
// 验证路径
router.post('/validate-path', auth_1.authenticateToken, async (req, res) => {
    try {
        const { path: pathToValidate } = req.body;
        if (!pathToValidate) {
            return res.status(400).json({ error: '请提供路径' });
        }
        const exists = fs_1.default.existsSync(pathToValidate);
        const isDirectory = exists && fs_1.default.statSync(pathToValidate).isDirectory();
        const isWritable = exists && isDirectory && fs_1.default.accessSync(pathToValidate, fs_1.default.constants.W_OK) === undefined;
        res.json({
            exists,
            isDirectory,
            isWritable,
            absolutePath: path_1.default.resolve(pathToValidate),
        });
    }
    catch (error) {
        res.json({
...
// 获取自动导入状态
router.get('/auto-import/status', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        if (fs_1.default.existsSync(importDir)) {
            ...
            filesCount = files.filter(f => {
                const filePath = path_1.default.join(importDir, f);
            ...
            supportedFilesCount = files.filter(f => {
                ...
                const filePath = path_1.default.join(importDir, f);
                return fs_1.default.statSync(filePath).isFile() && supportedExts.includes(ext);
            }).length;
        }
        res.json({
            ...status,
...
// 手动触发扫描
router.post('/auto-import/scan', auth_1.authenticateToken, async (req, res) => {
    try {
...
// 测试邮件发送（仅管理员）
router.post('/test-email', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
