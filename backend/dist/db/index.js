...
function initDatabase() {
  ...
  `);
    // 检查并添加新字段（用于数据库迁移）
    try {
        const booksTableInfo = exports.db.prepare("PRAGMA table_info(books)").all();
        const usersTableInfo = exports.db.prepare("PRAGMA table_info(users)").all();
        ...
        if (!hasFileHash) {
            exports.db.exec('ALTER TABLE books ADD COLUMN file_hash TEXT');
            console.log('已添加 file_hash 字段');
        }
        if (!hasCategory) {
            exports.db.exec("ALTER TABLE books ADD COLUMN category TEXT DEFAULT '未分类'");
            console.log('已添加 category 字段');
        }
        if (!hasUploaderId) {
            exports.db.exec('ALTER TABLE books ADD COLUMN uploader_id TEXT');
            console.log('已添加 uploader_id 字段');
        }
        if (!hasIsPublic) {
            exports.db.exec('ALTER TABLE books ADD COLUMN is_public INTEGER DEFAULT 0');
            console.log('已添加 is_public 字段');
        }
        if (!hasParentBookId) {
            exports.db.exec('ALTER TABLE books ADD COLUMN parent_book_id TEXT');
            console.log('已添加 parent_book_id 字段');
        }
        if (!hasRole) {
            exports.db.exec("ALTER TABLE users ADD COLUMN role TEXT DEFAULT 'user'");
            console.log('已添加 role 字段');
        }
        // 注意：系统会在第一个用户注册时自动设置为管理员
        // 检查是否有管理员
        try {
            const adminCount = exports.db.prepare("SELECT COUNT(*) as count FROM users WHERE role = 'admin'").get();
            const totalUsers = exports.db.prepare("SELECT COUNT(*) as count FROM users").get();
            if (totalUsers.count > 0 && adminCount.count === 0) {
                // 如果有用户但没有管理员，将第一个用户设置为管理员
                const firstUser = exports.db.prepare('SELECT id, username FROM users ORDER BY created_at ASC LIMIT 1').get();
                if (firstUser) {
                    exports.db.prepare("UPDATE users SET role = 'admin' WHERE id = ?").run(firstUser.id);
                    console.log('========================================');
  ...
  `);
    // 检查并添加新字段（用于数据库迁移）
    try {
        const tableInfo = exports.db.prepare("PRAGMA table_info(reading_progress)").all();
        ...
        if (!hasCurrentPage) {
            exports.db.exec('ALTER TABLE reading_progress ADD COLUMN current_page INTEGER DEFAULT 1');
            console.log('已添加 current_page 字段');
        }
        if (!hasTotalPages) {
            exports.db.exec('ALTER TABLE reading_progress ADD COLUMN total_pages INTEGER DEFAULT 1');
            console.log('已添加 total_pages 字段');
        }
        if (!hasChapterIndex) {
            exports.db.exec('ALTER TABLE reading_progress ADD COLUMN chapter_index INTEGER DEFAULT 0');
            console.log('已添加 chapter_index 字段');
        }
        if (!hasScrollTop) {
            exports.db.exec('ALTER TABLE reading_progress ADD COLUMN scroll_top REAL DEFAULT 0');
  ...
  `);
    ...
    try {
        const tableInfo = exports.db.prepare("PRAGMA table_info(reading_history)").all();
        ...
        if (!hasLastReadAt && tableInfo.some((col) => col.name === 'read_at')) {
            exports.db.exec('ALTER TABLE reading_history RENAME COLUMN read_at TO last_read_at');
            console.log('已迁移 reading_history.read_at 到 last_read_at');
        }
        if (!hasTotalReadingTime) {
            exports.db.exec('ALTER TABLE reading_history ADD COLUMN total_reading_time INTEGER DEFAULT 0');
            console.log('已添加 total_reading_time 字段');
        }
        if (!hasTotalProgress) {
            exports.db.exec('ALTER TABLE reading_history ADD COLUMN total_progress REAL DEFAULT 0');
            console.log('已添加 total_progress 字段');
        }
        if (!hasReadCount) {
            exports.db.exec('ALTER TABLE reading_history ADD COLUMN read_count INTEGER DEFAULT 0');
            console.log('已添加 read_count 字段');
        }
        // 检查并添加 updated_at 字段
        const hasUpdatedAt = tableInfo.some((col) => col.name === 'updated_at');
        if (!hasUpdatedAt) {
            exports.db.exec('ALTER TABLE reading_history ADD COLUMN updated_at DATETIME');
            // 为现有记录设置 updated_at = last_read_at
            exports.db.exec('UPDATE reading_history SET updated_at = last_read_at WHERE updated_at IS NULL');
  ...
  `);
    ...
    try {
        const notesTableInfo = exports.db.prepare("PRAGMA table_info(notes)").all();
      ...
      `);
            // 复制数据
            exports.db.exec('INSERT INTO notes_new SELECT * FROM notes');
            // 删除旧表
            exports.db.exec('DROP TABLE notes');
            // 重命名新表
            exports.db.exec('ALTER TABLE notes_new RENAME TO notes');
            // 重新启用外键检查
            exports.db.pragma('foreign_keys = ON');
  ...
  `);
    // 初始化默认书籍类型
    try {
        const existingCategories = exports.db.prepare('SELECT COUNT(*) as count FROM book_categories').get();
        if (existingCategories.count === 0) {
            ...
            const stmt = exports.db.prepare('INSERT INTO book_categories (id, name, display_order) VALUES (?, ?, ?)');
            defaultCategories.forEach((name, index) => {
                stmt.run((0, uuid_1.v4)(), name, index);
            });
  ...
  `);
    ...
    const insertSetting = exports.db.prepare('INSERT OR IGNORE INTO system_settings (id, key, value, description) VALUES (?, ?, ?, ?)');
    const updateSettingValue = exports.db.prepare('UPDATE system_settings SET value = ? WHERE key = ?');
    const updateSettingDesc = exports.db.prepare('UPDATE system_settings SET description = ? WHERE key = ?');
    defaultSettings.forEach((setting) => {
        // 检查设置是否已存在
        const existing = exports.db.prepare('SELECT value, description FROM system_settings WHERE key = ?').get(setting.key);
        if (existing) {
            // 如果已存在，只更新描述（如果不同）
            ...
            // 如果不存在，插入新设置
            insertSetting.run((0, uuid_1.v4)(), setting.key, setting.value, setting.description);
        }
    });
    // 创建索引
    exports.db.exec(`
    ...
    try {
        ...
        if (totalUsers.count === 0) {
            ...
            try {
                ...
                exports.db.prepare('INSERT INTO users (id, username, email, password, role) VALUES (?, ?, ?, ?, ?)').run(userId, defaultUsername, defaultEmail, hashedPassword, 'admin');
                // 设置私人访问密钥
                const privateKeySettingExists = exports.db.prepare('SELECT id FROM system_settings WHERE key = ?').get('private_access_key');
                if (privateKeySettingExists) {
                    exports.db.prepare('UPDATE system_settings SET value = ? WHERE key = ?').run(defaultPrivateKey, 'private_access_key');
                }
                console.log('========================================');
