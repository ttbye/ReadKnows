...
exports.AutoImportHandler = void 0;
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const uuid_1 = require("uuid");
const epubParser_1 = require("./epubParser");
const pdfMetadataExtractor_1 = require("./pdfMetadataExtractor");
const pdfCoverExtractor_1 = require("./pdfCoverExtractor");
const epubConverter_1 = require("./epubConverter");
const fileHash_1 = require("./fileHash");
...
class AutoImportHandler {
    ...
    getSetting(key, defaultValue = '') {
        const setting = this.db.prepare('SELECT value FROM system_settings WHERE key = ?').get(key);
    ...
    async processFile(file) {
        ...
        try {
            ...
            if (fileExt === '.mobi' && autoConvertMobi) {
                ...
                try {
                    finalFilePath = await (0, epubConverter_1.convertMobiToEpub)(file.filePath);
                ...
                const title = path_1.default.basename(file.fileName, '.txt');
                try {
                    finalFilePath = await (0, epubConverter_1.convertTxtToEpub)(file.filePath, title, '未知作者');
            ...
            // 2. 计算文件哈希（检查重复）
            const fileHash = await (0, fileHash_1.calculateFileHash)(finalFilePath);
            ...
            // 3. 提取元数据（第一次，不保存封面）
            const finalFileExt = path_1.default.extname(finalFilePath).toLowerCase();
            if (finalFileExt === '.epub') {
                try {
                    metadata = await (0, epubParser_1.extractEpubMetadata)(finalFilePath);
                    ...
                    metadata = {
                        title: path_1.default.basename(file.fileName, fileExt),
                        author: '未知作者',
                    };
                }
            }
                ...
                try {
                    ...
                    metadata = {
                        title: pdfMetadata.title || path_1.default.basename(file.fileName, fileExt),
            ...
            const bookDir = path_1.default.join(booksDir, 'public', bookId);
            // 确保目录存在
            ...
            const newFilePath = path_1.default.join(bookDir, newFileName);
            fs_1.default.copyFileSync(finalFilePath, newFilePath);
            console.log('[自动导入] 文件已复制到:', newFilePath);
            // 6. 提取封面（第二次，保存封面文件）
            if (finalFileExt === '.epub') {
                try {
                    console.log('[自动导入] 开始提取EPUB封面，bookDir:', bookDir);
                    const coverMetadata = await (0, epubParser_1.extractEpubMetadata)(newFilePath, bookDir);
                ...
                try {
                    ...
                    const coverUrl = await (0, pdfCoverExtractor_1.extractPdfCover)(newFilePath, bookDir);
                    if (coverUrl) {
            ...
            // 7. 保存到数据库
            const fileSize = fs_1.default.statSync(newFilePath).size;
        ...
        `)
                .run(bookId, metadata.title || '未知标题', metadata.author || '未知作者', metadata.description || null, metadata.cover_url || null, newFilePath, newFileName, fileSize, finalFileExt.substring(1), // 去掉点号
            fileHash, 1, // is_public
...
exports.AutoImportHandler = AutoImportHandler;
//# sourceMappingURL=autoImportHandler.js.map