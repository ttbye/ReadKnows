...
router.post('/progress', authenticateToken, async (req: AuthRequest, res) => {
  try {
    ...
    const bookExists = db.prepare('SELECT id FROM books WHERE id = ?').get(bookId) as any;
    if (!bookExists) {
    ...
    if (existing && !force) {
      ...
      const progressId = uuidv4();
        ...
        if (insertError.code === 'SQLITE_CONSTRAINT_FOREIGNKEY') {
          ...
          const userCheck = db.prepare('SELECT id, username FROM users WHERE id = ?').get(userId) as any;
          const bookCheck = db.prepare('SELECT id, title FROM books WHERE id = ?').get(bookId) as any;
    ...
    try {
      ...
      const currentProgressRecord = db
      ...
      if (existingHistory) {
        ...
        const updateHistoryResult = db.prepare(`
            ...
            if (duration > 0 && duration < 1) {
              ...
              if (activeSession) {
                ...
                `).run(
                  ...
                  oldLastReadAt,
                  duration,
                  existingHistory.total_progress || 0,
                  actualProgress
                );
...
// 获取阅读进度
router.get('/progress/:bookId', authenticateToken, async (req: AuthRequest, res) => {
  try {
...
// 保存阅读设置
router.post('/settings', authenticateToken, async (req: AuthRequest, res) => {
  try {
    const userId = req.userId!;
  ...
  try {
    ...
    if (bookId) {
      ...
      const bookPref = db
        ...
        .get(userId, bookId, fileType || 'epub') as any;
    ...
    const preferences = db.prepare(query).all(...params) as any[];
...
router.post('/preferences', authenticateToken, async (req: AuthRequest, res) => {
  try {
    ...
    const existing = db
      ...
      .get(userId, bookId || null, fileType) as any;
...
// 删除阅读器偏好设置（恢复默认值）
router.delete('/preferences', authenticateToken, async (req: AuthRequest, res) => {
  try {
    const userId = req.userId!;
    const { bookId, fileType } = req.query;
...
// 获取单本书的详细阅读历史（包括所有会话）
router.get('/history/:bookId', authenticateToken, async (req: AuthRequest, res) => {
  try {
    const userId = req.userId!;
    const { bookId } = req.params;
...
// 删除阅读历史
router.delete('/history/:bookId', authenticateToken, async (req: AuthRequest, res) => {
  try {
    const userId = req.userId!;
    const { bookId } = req.params;
    ...
    // 删除记录（级联删除会同时删除所有会话）
    db.prepare('DELETE FROM reading_history WHERE id = ?').run(history.id);
...
// 获取用户阅读统计
router.get('/history/stats/summary', authenticateToken, async (req: AuthRequest, res) => {
  try {
    const userId = req.userId!;
    ...
    const yearReadingTime = db
      .prepare(`
        SELECT COALESCE(SUM(duration), 0) as total
        FROM reading_sessions
        WHERE user_id = ? AND start_time >= ?
      `)
      .get(userId, yearStart) as any;
...
// 创建阅读会话（由前端在开始阅读时调用）
router.post('/history/session', authenticateToken, async (req: AuthRequest, res) => {
  try {
    const userId = req.userId!;
    ...
    const existingActiveSession = db
      .prepare(`
        SELECT id FROM reading_sessions 
        WHERE history_id = ? AND end_time IS NULL AND start_time > ?
        ORDER BY start_time DESC LIMIT 1
      `)
      .get(history.id, oneHourAgo) as any;
...
// 结束阅读会话（由前端在结束阅读时调用）
router.put('/history/session/:sessionId', authenticateToken, async (req: AuthRequest, res) => {
  try {
    const userId = req.userId!;
    ...
    if (finalProgressAfter === undefined || finalProgressAfter === null) {
      try {
        const currentProgress = db
          .prepare('SELECT progress FROM reading_progress WHERE user_id = ? AND book_id = ?')
          .get(userId, session.book_id) as any;
    ...
    // 更新会话
    try {
      const updateSessionResult = db.prepare(`
        UPDATE reading_sessions
        SET end_time = ?,
            duration = ?,
            progress_after = ?
    ...
    let previousTotal = 0;
    try {
      const allSessionsTotal = db
        .prepare(`
          SELECT COALESCE(SUM(duration), 0) as total
          FROM reading_sessions
          WHERE history_id = ? AND end_time IS NOT NULL AND id != ?
        `)
        .get(session.history_id, sessionId) as any;
